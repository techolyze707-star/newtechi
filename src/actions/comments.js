...
import { checkRateLimit, getClientIp, validateSameOrigin } from '@/lib/form-security';
...
export async function createComment(documentId, name, content) {
  try {
    ...
    const originCheck = validateSameOrigin(headerStore);
    if (!originCheck.ok) {
    ...
    if (!rateLimit.allowed) {
      return { success: false, error: 'Too many comments. Please wait a moment and try again.' };
    }
