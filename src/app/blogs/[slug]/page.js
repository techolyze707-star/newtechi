...
export async function generateMetadata({ params }) {
  ...
  if (!result.success) {
    return {
      title: 'Blog Not Found - Techolyze',
...
export default async function BlogPostPage({ params }) {
 ...
 return (
    <>
      ...
      <main className="min-h-screen bg-[#171717]" aria-labelledby="blog-title" itemScope itemType="https://schema.org/BlogPosting">
        <div className="max-w-7xl mx-auto px-0 sm:px-6 lg:px-12 py-8">

          {/* Breadcrumbs */}
          <nav aria-label="Breadcrumb" className="mb-6 px-2 flex flex-wrap items-center text-sm text-white">
            <Link
              href="/"
              prefetch={false}
            ...
            <ChevronRight size={16} className="mx-1 text-white" aria-hidden="true" />
            <Link
              href="/blogs"
              prefetch={false}
              className="hover:text-yellow-400 transition-colors"
            ...
            <ChevronRight size={16} className="mx-1 text-white" aria-hidden="true" />
            <span className="text-white truncate max-w-[200px] sm:max-w-md" aria-current="page">
              Blog
            </span>
          </nav>
          ...
          <header className="mb-8 py-6" aria-label="Blog post header">
            ...
            <h1
              id="blog-title"
              className="text-3xl sm:text-4xl md:text-5xl px-3 font-normal text-white leading-tight mb-6"
              itemProp="headline"
            >
              {displayTitle}
            </h1>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start gap-4 mt-6 px-3">
              ...
              {displayExcerpt && (
                <div className="border-l border-white pl-4 flex-1">
            ...
            <div className="lg:col-span-8">
              ...
              >
                <div className="py-4 sm:py-6 overflow-hidden">
                  ...
                  <meta itemProp="author" content={blog.author?.name || 'Techolyze'} />
                  <meta itemProp="publisher" content="Techolyze" />
                  {blog.updatedAt && (
          ...
          {relatedBlogs.length > 0 && (
            <section className="mt-20 pt-12 border-t border-white max-sm:px-4" aria-label="Related blog posts">
              <div className="flex items-center justify-between mb-8">
                ...
                <Link
                  href="/blogs"
                  prefetch={false}
                  className="text-yellow-400 hover:text-white font-medium text-sm flex items-center gap-1 transition-colors"
                >
                  View all
